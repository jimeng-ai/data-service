...
class GlobalSearchServiceTest {
    ...
    private GlobalSearchService newService(SkillTenantService skills, PermissionResolver resolver) {
        ...
        return new GlobalSearchService(agentService, kbService, kbDocumentMapper,
                aiTraceMapper, resolver, skills);
    }

    @Test
    void matchesSkillByDescription() {
        SkillTenantService skills = Mockito.mock(SkillTenantService.class);
        when(skills.list(any(), any(), eq(false))).thenReturn(List.of(
                skill(1, "无关名字", "用于查询天气预报"),
                skill(2, "另一个", "发送邮件")));
        PermissionResolver resolver = Mockito.mock(PermissionResolver.class);

        GlobalSearchResult r = newService(skills, resolver).search("天气", 5);

        assertThat(r.getSkills()).extracting(GlobalSearchResult.SkillHit::getId).containsExactly(1L);
    }
    ...
    void limitsSkillResults() {
        ...
        GlobalSearchResult r = newService(skills, resolver).search("报告", 3);
