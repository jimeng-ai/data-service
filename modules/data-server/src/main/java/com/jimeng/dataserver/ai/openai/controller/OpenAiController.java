...
public class OpenAiController {
    ...
    @Operation(summary = "OpenAI聊天对话", description = "对接OpenAI /v1/chat/completions，默认模型gpt-5.3-codex")
    ...
    public Object chatCompletions(@RequestBody Map<String, Object> requestBody,
        ...
        if (Boolean.TRUE.equals(requestBody.get("stream"))) {
            return chatCompletionsStream(requestBody, request);
        }
        return openAiService.chatCompletions(requestBody);
    }

    private SseEmitter chatCompletionsStream(Map<String, Object> requestBody,
                                             HttpServletRequest request) {
        String connectionId = UUID.randomUUID().toString();
        String traceId = extractTraceId(request);
        SseEmitter emitter = sseServiceUtil.getConnection(connectionId, 300_000L);
        CompletableFuture.runAsync(() -> openAiService.chatCompletionsStream(requestBody, connectionId, traceId));
        return emitter;
    }
